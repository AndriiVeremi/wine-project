...
Щоб переконатися, що API працює швидко, ми використовуємо **Autocannon**.

### 1. Тестування API (Бекенд)
Для запуску тесту необхідно, щоб бекенд був запущений (локально або в Docker).

```bash
cd backend
# Запустить базовий тест для списку вин
npm run benchmark
```

Ви також можете протестувати будь-який інший ендпоінт вручну:
```bash
npx autocannon -c 100 -d 10 http://localhost:5005/api/wineries
```
*Де `-c` — кількість одночасних з'єднань, `-d` — тривалість тесту в секундах.*

### 2. Аналіз збірки (Фронтенд)
Щоб побачити розмір файлів та зрозуміти, які бібліотеки займають найбільше місця:
```bash
cd frontend
npm run build
```
Після збірки Vite виведе таблицю з розмірами всіх чанків.
...
| :--- | :--- | :--- |
| `AI_ASSISTANT_ENABLED` | Головний перемикач ШІ. | `true` |
| `GEMINI_API_KEY` | Ключ від Google AI Studio. | `AIzaSy...` |
| `GEMINI_MODEL_NAME` | Модель Gemini (напр. `gemini-2.5-flash`, `gemini-3-flash-preview` або `gemma-4-31b-it`). | 
| `GEMINI_MAX_OUTPUT_TOKENS` | Максимальна довжина відповіді. | `500` |
| `AI_MAX_FUNCTION_CALLS` | Максимальна кількість викликів інструментів за один запит. | `5` |

### Важливо для стабільної роботи
- **Тайм-аути**: На фронтенді встановлено загальний тайм-аут 30 секунд для API та спеціальний 60 секунд для ШІ запитів (щоб виключити `AxiosError: timeout` при складних викликах функцій).
- **Очищення пам'яті**: Чат використовує `sessionStorage`. Історія повідомлень (ліміт 20) зберігається протягом сесії та автоматично видаляється при закритті вкладки.
- **Очищення виводу**: Бекенд автоматично фільтрує відповіді ШІ, вилучаючи технічні плани та роздуми (Chain of Thought) за допомогою системи тегів `<msg>`.
