...
| **React Router 7** | Маршрутизація (повна підтримка v7) |
| **TanStack Query 5** | Серверний стейт, кешування, синхронізація |
| **Zustand 5** | Клієнтський UI-стейт (фільтри, авторизація) |
...
AI-Сомельє побудований на базі **Google Gemini SDK** (модель `gemini-2.5-flash`) з використанням технології **Function Calling**. Модель має доступ до інструментів бази даних, що гарантує достовірність рекомендацій.
...
- `searchWines`: Пошук вин за кольором, солодкістю, ціною та рейтингом.
- `getRegionInfo`: Отримання детальної інформації про винні регіони.
- `getWineryInfo`: Дані про історію та контакти виноробень.
- `searchTours`: Пошук доступних екскурсій та дегустацій.
- `searchGrapes`: Докладна інформація про сорти винограду.
- `getMyFavoriteWines`: Доступ до списку улюблених вин користувача для персоналізованих порад.
